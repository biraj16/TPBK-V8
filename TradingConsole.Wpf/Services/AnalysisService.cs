...
namespace TradingConsole.Wpf.Services
{
    public enum MarketThesis
    {
        ...
        Bullish_Reversal_At_Support,
        Bullish_Breakout_Attempt,

        // Bearish Scenarios
        Bearish_Trend_Continuation,
        Bearish_Reversal_At_Resistance,
        Bearish_Breakdown_Attempt,

        // Neutral / Indeterminate Scenarios
        Balancing_Range_Bound,
    ...
    public class AnalysisService : INotifyPropertyChanged
    {
        ...
        private readonly MarketInternalsService _marketInternalsService; // NEW
        ...
        public AnalysisService(SettingsViewModel settingsViewModel, DhanApiClient apiClient, ScripMasterService scripMasterService, HistoricalIvService historicalIvService, MarketProfileService marketProfileService, IndicatorStateService indicatorStateService, SignalLoggerService signalLoggerService, NotificationService notificationService, DashboardViewModel dashboardViewModel, MainViewModel mainViewModel)
        {
            ...
            _marketInternalsService = new MarketInternalsService(settingsViewModel);
        ...
        public void OnInstrumentDataReceived(DashboardInstrument instrument, decimal underlyingPrice)
        {
            ...
            if (!_stateManager.BackfilledInstruments.Contains(instrument.SecurityId))
            {
                ...
                _ = InitializeNewInstrument(instrument);
            }
        ...
        private void RunComplexAnalysis(DashboardInstrument instrument, bool newCandleFormed)
        {
            ...
            // NEW: Calculate participation score for Nifty
            if (instrument.InstrumentType == "INDEX" && instrument.Symbol == "Nifty 50")
            {
                var allInstruments = _mainViewModel.Dashboard.MonitoredInstruments;
                result.ParticipationScore = _marketInternalsService.CalculateParticipationScore(instrument, allInstruments);
            }
        ...
        public AnalysisResult? GetResultForSymbol(string symbol)
        {
            return _stateManager.AnalysisResults.Values.FirstOrDefault(r => r.Symbol == symbol);
        }
        ...
        private async Task BackfillCurrentDayCandlesAsync(DashboardInstrument instrument)
        {
            ...
            try
            {
                // --- THE FIX: Use the Futures contract for volume data when backfilling an index ---
                ...
                var priceScripInfo = _scripMasterService.FindBySecurityId(instrumentForHistory.SecurityId);
                ...
                // The SecurityId of the original instrument (the index) is used as the key
        ...
        private async Task BackfillAndSavePreviousDayProfileAsync(DashboardInstrument instrument)
        {
            ...
            {
                DashboardInstrument instrumentForHistory = instrument;
                ScripInfo? scripInfoForHistory = null;

                // --- THE FIX: Ensure futures data is processed first ---
                if (instrument.InstrumentType == "INDEX")
                {
                    var futureInstrument = _instrumentCache.Values.FirstOrDefault(i => i.IsFuture && i.UnderlyingSymbol == instrument.Symbol);
                    if (futureInstrument != null)
                    {
                        // Recursively call to ensure the future is backfilled first.
                        // The check at the start of the method prevents infinite loops.
                        await BackfillAndSavePreviousDayProfileAsync(futureInstrument);
                        instrumentForHistory = futureInstrument; // Use future for volume data
                    }
                }

                scripInfoForHistory = _scripMasterService.FindBySecurityId(instrumentForHistory.SecurityId);
        ...
        {
            return minuteCandles.GroupBy(c => new DateTime(c.Timestamp.Ticks - (c.Timestamp.Ticks % timeframe.Ticks), DateTimeKind.Utc))
                .Select(g => new Candle
                {
                    Timestamp = g.Key,
                    Open = g.First().Open,
        ...
        private void UpdateMarketProfileForCandle(DashboardInstrument instrument, Candle lastClosedCandle)
        {
            ...
            {
                // This part of the logic for futures is correct.
            ...
            else // This is the block for when the instrument is an INDEX
            {
                if (_stateManager.MarketProfiles.TryGetValue(instrument.SecurityId, out var indexProfile))
                {
                    // --- MODIFIED LOGIC ---
                    // Find the future contract with a more robust check for Nifty.
                    var future = _instrumentCache.Values.FirstOrDefault(i =>
                        i.IsFuture &&
                        (i.UnderlyingSymbol == instrument.UnderlyingSymbol ||
                         (instrument.Symbol == "Nifty 50" && i.UnderlyingSymbol == "NIFTY")));

                    if (future != null)
                    {
                        var futureCandles = _stateManager.GetCandles(future.SecurityId, TimeSpan.FromMinutes(1));

                        var matchingFutureCandle = futureCandles?
                            .OrderBy(c => Math.Abs((c.Timestamp - lastClosedCandle.Timestamp).TotalSeconds))
                            .FirstOrDefault();

                        if (matchingFutureCandle != null && Math.Abs((matchingFutureCandle.Timestamp - lastClosedCandle.Timestamp).TotalSeconds) < 10)
                        {
                            _signalGenerationService.UpdateMarketProfile(indexProfile, lastClosedCandle, matchingFutureCandle);
                        }
                        else
                        {
                            _signalGenerationService.UpdateMarketProfile(indexProfile, lastClosedCandle,
                                new Candle { Volume = 0, High = lastClosedCandle.High, Low = lastClosedCandle.Low });
                        }
                    }
                }
            }
